...
export class GameBoyCatridge {
    ...
    constructor(rawGame: ArrayBuffer, register?: Record<TFormKey, string>) {
        ...
        if (poop.isPrefix) {
            this.CartDataToBytes[parseInt(register.pc)] = 0xcb;
            this.CartDataToBytes[parseInt(register.pc) + 1] = poop.value;
            this.CartDataToBytes[parseInt(register.pc) + 2] = parseInt(register.ub, 16);
            this.CartDataToBytes[parseInt(register.pc) + 3] = parseInt(register.lb, 16);
        } else {
            console.log('HELLO' + ' ' + poop.value);
            this.CartDataToBytes[parseInt(register.pc, 16)] = poop.value;

            console.log(this.CartDataToBytes[parseInt(register.pc, 16)]);
            this.CartDataToBytes[parseInt(register.pc, 16) + 1] = parseInt(register.ub, 16);
            this.CartDataToBytes[parseInt(register.pc, 16) + 2] = parseInt(register.lb, 16);
        }
        this.CartDataToBytes[parseInt(register.addressPointer, 16)] = parseInt(
    ...
    getCartridgeHeaderRaw() {
        const cartridgeType = this.#CartData.getUint8(CartHeaderAdress.Adresses.cartridgeType[0]);

        const cartridgeRomSize = this.#CartData.getUint8(CartHeaderAdress.Adresses.romSize[0]);

        const cartridgeRamSize = this.#CartData.getUint8(CartHeaderAdress.Adresses.ramSize[0]);
        ...
        return {
            ...
            maskRomVersionNumber: GameBoyCatridge.#ToHex(cartridgeMaskRomVersionNumber),
