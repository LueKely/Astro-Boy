...
function LDHLSPe() {
	return [
		(dmg: Gameboy) => {
			...
			dmg.registers.setLowerByte(lowerPC);
			dmg.registers.setUpperByte(pc >> 8);
			dmg.registers.setTempByte(pc + lowerPC);
		},

		(dmg: Gameboy) => {
			const targetValue = dmg.registers.getTempByte();
		...
		(dmg: Gameboy) => {
			const adj = dmg.registers.getTempByte() != 0 ? 1 : 0;
			const result =
