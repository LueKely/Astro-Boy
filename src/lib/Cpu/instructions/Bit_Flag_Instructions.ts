...
// BIT U3, [HL]
function BITU3HL(u3: number, HL: Cpu_Register_16Bit<'HL'>, ram: Ram, f: Cpu_Flag_Register) {
    const testBit = (ram.read(HL.getRegister()) >>> u3) & 0b1;
    validateBitFlagOperation(testBit, f);
    //   add validation flag here
}
...
function RESU3HL(u3: number, HL: Cpu_Register_16Bit<'HL'>, ram: Ram) {
    const testBit = ram.read(HL.getRegister()) & ~(0b01 << u3);
    ram.write(HL.getRegister(), testBit);
}
